...
export function SalesScreen() {
  ...
  unit === "dozen" ? quantity * 12 : quantity;

  const hasEnoughStock =
    !selectedProduct ||
    quantityInPairs <= selectedProduct.currentStock;
  ...
  function addLine() {
    ...
    setProductId(""); setSelectedProduct(null); setQuantity(1); setMessage(null);
  }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await createSale({ customerName: String(data.get("customerName")).trim(), phone: String(data.get("phone")).trim() || null, observation: String(data.get("observation")).trim() || null, items: lines.map((line) => ({ productId: line.product.id, quantity: line.quantityInPairs })) });
  ...
  <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
    ...
    <div className="grid gap-3 md:grid-cols-[120px_140px_auto]">
      ...
      <select
        value={unit}
        onChange={(event) =>
          setUnit(event.target.value as SaleUnit)
        }
        className={inputClass.replace("mt-1 ", "")}
      >
    ...
    <form onSubmit={handleSubmit} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="grid gap-4 md:grid-cols-3"><label className="text-sm font-medium text-slate-700">Cliente<input required name="customerName" className={inputClass} /></label><label className="text-sm font-medium text-slate-700">Teléfono<input name="phone" className={inputClass} /></label><label className="text-sm font-medium text-slate-700">Observación<input name="observation" className={inputClass} /></label></div> 
    <div className="mt-5">
  {lines.length === 0 ? (
          ...
          {lines.map((line) => (
            <tr key={line.product.id}>
              ...
              <td className="px-3 py-3 text-right font-semibold">
                {money.format(
                  (line.product.salePrice ?? 0) *
                    line.quantityInPairs
                )}
              </td>
